...
class Status:
  def __init__(self, detect, clk, select, track):
    self.DETECTING = detect
    self.CLICKED = clk
    self.SELECTED = select
    self.TRACKING = track

status = Status(True, False, False, False)
bbox = []
selected_bbox = []
...
for i in range(len(res)):
  print(f'Computation time Case {i+1}: {computation_times[i]}')